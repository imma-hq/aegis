...
export class GroupManager {
  ...
  private sentMessageNumbers: Map<string, Map<string, number>>; // groupId -> senderId -> messageNumber
  ...
  async createGroup(
    ...
    const memberPublicKeysMap = new Map<string, Uint8Array>();
    const memberDsaPublicKeysMap = new Map<string, Uint8Array>();
    ...
    const group: Group = {
      ...
      receivedMessageNumbers: new Map<string, number>(), // Initialize received message numbers tracking
    };
  ...
  async encryptMessage(
    ...
    if (!this.sentMessageNumbers) {
      this.sentMessageNumbers = new Map<string, Map<string, number>>(); // groupId -> senderId -> messageNumber
    }

    if (!this.sentMessageNumbers.has(groupId)) {
      this.sentMessageNumbers.set(groupId, new Map<string, number>());
    }

    const groupSentNumbers = this.sentMessageNumbers.get(groupId)!;
    const currentMessageNumber =
      (groupSentNumbers.get(this.identity.userId) || 0) + 1;
  ...
  async getGroup(groupId: string): Promise<Group | null> {
    ...
    const groupData = (session as any).groupData;
    if (!groupData) {
      return null;
    }
    ...
    const receivedMessageNumbers = new Map<string, number>();
  ...
  private serializeGroupHeader(header: GroupMessageHeader): Uint8Array {
