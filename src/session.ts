...
export class SessionManager {
  ...
  constructor(aegisInstance: Aegis) {
    ...
    if (!aegisInstance.isInitialized()) {
      throw new SessionError(
        "Aegis instance must be initialized before creating SessionManager",
      );
    }
  }
  ...
  private deriveX25519SharedSecret(
    privateKey: Uint8Array,
    publicKey: Uint8Array,
  ): Uint8Array {
  ...
  private storeSkippedMessageKey(
    ...
    const totalSkipped = Object.values(state.skippedMessageKeys).reduce(
      (acc, cur) => acc + Object.keys(cur).length,
      0,
    );
    if (totalSkipped >= (state.skippedMessagesLimit ?? MAX_SKIPPED_MESSAGES)) {
      throw new SessionError(
        "Skipped message limit exceeded",
        state.sessionId,
        { totalSkipped, limit: state.skippedMessagesLimit },
      );
    }
  ...
  private getAndRemoveSkippedMessageKey(
    state: SessionState,
    ratchetPubKeyBase64: string,
    messageNumber: number,
  ): Uint8Array | null {
    if (!state.skippedMessageKeys) return null;
    const bucket = state.skippedMessageKeys[ratchetPubKeyBase64];
  ...
  private deriveMessageKeyAndNextChain(
    chainKey: Uint8Array,
    index: number,
  ...
  private deriveRootFromSS(rootKey: Uint8Array, ss: Uint8Array): Uint8Array {
  ...
  private async performDHratchet(
    ...
    for (let i = state.receiveMessageNumber; i < (pn || 0); i++) {
      const { messageKey, nextChainKey } = this.deriveMessageKeyAndNextChain(
        chainKey,
        i,
      );
  ...
  async initializeSession(
    ...
    const recipientSignedPreKey = base64ToBytes(
    ...
    const dh1 = this.deriveX25519SharedSecret(
    ...
    const dh2 = this.deriveX25519SharedSecret(
      ephemeralKeyPair.privateKey,
  ...
  async decryptMessage(encryptedMsg: EncryptedMessage): Promise<string> {
    ...
    return this.withSessionLock(encryptedMsg.sessionId, async () => {
      ...
      if (theirRatchetPubBase64) {
        const skipped = this.getAndRemoveSkippedMessageKey(
          state,
          theirRatchetPubBase64,
          encryptedMsg.messageNumber,
        );
        if (skipped) {
          try {
            const plaintextBytes = decrypt(
              skipped,
              base64ToBytes(encryptedMsg.nonce),
              base64ToBytes(encryptedMsg.ciphertext),
            );
            ...
            console.warn(
              `[Session] Skipped key decryption failed for message ${encryptedMsg.messageNumber}`,
              err,
            );
          }
        }
      ...
      if (encryptedMsg.messageNumber < state.receiveMessageNumber) {
        throw new SessionError(
          "Message number too old - possible replay attack",
          encryptedMsg.sessionId,
          {
      ...
      for (
        ...
        if (!theirRatchetPubBase64) {
          throw new SessionError(
            "Missing remote ratchet public key context for skipped messages",
            encryptedMsg.sessionId,
          );
        }
      ...
      try {
        ...
        throw new SessionError(
          "Message decryption failed - authentication error",
          encryptedMsg.sessionId,
  ...
  private async loadSessionState(
    ...
    try {
      ...
      let dhPrivate: Uint8Array;
      let dhPublic: Uint8Array;
      if (data.dhPrivate) {
