...
__export(index_exports, {
  ...
  getAndConsumePublicKeyBundle: () => getAndConsumePublicKeyBundle,
  getGroupSession: () => getGroupSession,
...
async function generateSignatureKeyPair() {
  const secretKey = import_ed25519.ed25519.utils.randomSecretKey();
  const publicKey = await import_ed25519.ed25519.getPublicKey(secretKey);
  return {
    publicKey,
    secretKey
  };
...
async function generateSignedPreKey(signingSecretKey) {
  ...
  if (signingSecretKey && signingSecretKey.length > 0) {
    signature = await import_ed25519.ed25519.sign(keyPair.publicKey, signingSecretKey);
  } else {
...
async function verifySignedPreKey(spkPublicKey, signature, signerPublicKey) {
  try {
    return await import_ed25519.ed25519.verify(signature, spkPublicKey, signerPublicKey);
  } catch {
    return false;
  }
}
