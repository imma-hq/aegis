 ...
 */
export function hash(data, outputLength = 32) {
    if (!data) {
 ...
 */
export function deriveKey(data, context, outputLength = 32) {
    const input = typeof data === "string" ? stringToBytes(data) : data;
 ...
 */
export function encrypt(key, nonce, plaintext, associatedData) {
    if (key.length !== 32) {
 ...
 */
export function decrypt(key, nonce, ciphertext, associatedData) {
    if (key.length !== 32) {
        throw new Error("Key must be 32 bytes");
    }
    if (nonce.length !== 12) {
...
export function base64ToBytes(base64) {
    ...
    for (let i = 0; i < str.length; i += 4) {
        ...
        const triplet = (encoded1 << 18) | (encoded2 << 12) | (encoded3 << 6) | encoded4;
        if (p < bufferLength)
