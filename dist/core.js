...
import { ERRORS, RATCHET_AFTER_MESSAGES, MAX_SKIPPED_MESSAGES, REPLAY_WINDOW_SIZE, MAX_MESSAGE_AGE, MAX_STORED_MESSAGE_IDS, } from "./constants.js";
...
export class E2EE {
        ...
        try {
            ...
            if (shouldRatchet) {
                ...
                Logger.log("Ratchet", "Sent KEM ratchet", {
                    ...
                    newKeyHash: bytesToHex(result.newRatchetKeyPair.publicKey).substring(0, 16) +
                        "...",
                });
            }
            if (!session.sendingChain) {
            ...
            const { messageKey, newChain } = KemRatchet.symmetricRatchet(session.sendingChain);
            // Encrypt
    ...
    async decryptMessage(sessionId, encrypted) {
        try {
            ...
            const needsRatchet = this.needsReceivingRatchet(session, encrypted.header);
            if (needsRatchet && encrypted.header.kemCiphertext) {
                ...
                const result = KemRatchet.performKemRatchetDecapsulate(session.rootKey, encrypted.header.kemCiphertext, session.currentRatchetKeyPair.secretKey);
                // Update session
                session.rootKey = result.newRootKey;
                session.currentRatchetKeyPair = result.newRatchetKeyPair;
            ...
            const skippedKeyId = this.getSkippedKeyId(encrypted.header.ratchetPublicKey, encrypted.header.messageNumber);
            ...
            if (session.receivingChain &&
                ...
                for (const [msgNum, msgKey] of skippedKeys) {
                    const keyId = this.getSkippedKeyId(encrypted.header.ratchetPublicKey, msgNum);
